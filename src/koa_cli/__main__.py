...
def _setup(args: argparse.Namespace) -> int:
    ...
    default_python = _prompt(
        ...
        default=(existing.get("python_module") or (existing.get("modules") or {}).get("python") or ""),
        required=False,
    )
    ...
    default_cuda = _prompt(
        ...
        default=(existing.get("cuda_module") or (existing.get("modules") or {}).get("cuda") or ""),
        required=False,
    )
...
def _init_project(args: argparse.Namespace) -> int:
    ...
    _write_file(config_path, config_yaml, overwrite=args.force)
