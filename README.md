...
The CLI automatically discovers `koa-config.yaml` by walking up from the current working directory. Environment variables such as `KOA_USER`, `KOA_HOST`, `KOA_IDENTITY_FILE`, `KOA_REMOTE_ROOT`, `KOA_LOCAL_ROOT`, `KOA_DEFAULT_PARTITION`, `KOA_CUDA_VERSION`, `KOA_ENV_WATCH`, `KOA_PROXY_COMMAND`, and `KOA_DASHBOARD_BASE_URL` can override the saved configuration at runtime.
